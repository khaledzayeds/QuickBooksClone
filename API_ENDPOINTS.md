...
Current API contract for the Zayed Offline ERP desktop client.
...
```text
Offline v1 contract. Keep API, Flutter, docs, and smoke tests aligned when changing it.
```
...
- Default local database files are `zayed.db` and `zayed-dev.db`.
...
- Offline v1 target architecture is `Flutter Windows desktop -> bundled Local API -> SQLite`.
- SQL Server, sync, hosted, LAN, payroll, and advanced inventory are deferred unless explicitly validated and licensed.
- Current sync readiness is on document headers only, not line tables, and sync is not an Offline v1 production feature.
...
- SQL Server uses a separate EF Core migrations assembly: `Zayed.SqlServerMigrations`, but SQL Server is not Offline v1 production-ready.
...
- Bearer authentication is enabled for business endpoints. Login and startup/setup endpoints are the only intentional anonymous paths.
- Send `Authorization: Bearer <token>` after login.
- Permission middleware protects business operations by role/permission.
- License middleware protects licensed features such as backup/restore and deferred payroll.
...
```json
{
  ...
  "liveDatabasePath": "E:\\Systems\\Zayed\\Zayed\\Zayed.Api\\zayed-dev.db",
  "backupDirectory": "E:\\Systems\\Zayed\\Zayed\\Zayed.Api\\backups",
  "backupCount": 2
}
```
...
```json
{
  "items": [
    {
      "fileName": "zayed-manual-backup-20260419094500-smoke.db",
      "fullPath": "E:\\Systems\\Zayed\\Zayed\\Zayed.Api\\backups\\zayed-manual-backup-20260419094500-smoke.db",
      "sizeBytes": 389120,
...
```json
{
  "fileName": "zayed-manual-backup-20260419094500-before-upgrade.db",
  "fullPath": "E:\\Systems\\Zayed\\Zayed\\Zayed.Api\\backups\\zayed-manual-backup-20260419094500-before-upgrade.db",
  "sizeBytes": 389120,
  "createdAt": "2026-04-19T09:45:00+00:00",
...
```json
{
  ...
  "requestedBy": "admin",
  "reason": "Before posting engine update",
  "safetyBackupFileName": "zayed-safety-backup-20260419100000-pre-restore.db",
...
- Restore validates that the selected backup looks like a Zayed SQLite database.
...
```json
{
  "fileName": "zayed-imported-backup-20260419120000-client-copy.db",
  "fullPath": "E:\\Systems\\Zayed\\Zayed\\Zayed.Api\\backups\\zayed-imported-backup-20260419120000-client-copy.db",
  "sizeBytes": 389120,
...
- Import validates that the uploaded file looks like a Zayed SQLite database before it is accepted.
...
```json
{
  "items": [
    {
      "backupFileName": "zayed-manual-backup-20260419094500-before-upgrade.db",
      "backupFullPath": "E:\\Systems\\Zayed\\Zayed\\Zayed.Api\\backups\\zayed-manual-backup-20260419094500-before-upgrade.db",
      "restoredAt": "2026-04-19T10:00:02+00:00",
      "createdSafetyBackup": true,
      "safetyBackupFileName": "zayed-safety-backup-20260419100000-pre-restore.db",
      "requestedBy": "admin",
