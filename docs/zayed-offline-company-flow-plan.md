# Zayed Offline Company Flow & Setup Wizard Plan

This document is the shared implementation plan for ChatGPT/Codex work on the `zayed/offline` branch.

The goal is to stop patching the first-run flow and implement a clear Zayed-style offline desktop experience.

---

## 1. Non-negotiable flow

Zayed Offline must have three separate concepts and three separate routes:
...
## 2. Zayed-style No Company Open screen

The first screen should resemble Zayed "No Company Open".
...
Location: C:/Users/.../Documents/Zayed/Companies
...
- `Create a new company`: creates/selects a `.zayed` file, registers it, opens backend runtime, then goes to setup.
- `Open or restore an existing company`: file picker for `.zayed` and `.db`; if chosen, register/open it.
...
- Then improve UI to match Zayed.
...
6. Replace the launcher UI with a closer Zayed-style table/actions layout.
