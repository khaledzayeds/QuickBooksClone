...
public sealed class LocalApiProcessService
{
    ...
    public async Task<bool> TryEnsureStartedAsync(ApiConnectionSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        if (projectPath is null)
        {
            ...
            _lastError = "Could not locate Zayed.Api.csproj for managed local startup.";
            return false;
        }
    ...
    private static string? ResolveProjectPath(string? configuredPath)
    {
        ...
        {
            candidates.Add(Path.Combine(current.FullName, "Zayed.Api", "Zayed.Api.csproj"));
        ...
        {
            candidates.Add(Path.Combine(working.FullName, "Zayed.Api", "Zayed.Api.csproj"));
    ...
    private static StartupCommand ResolveStartupCommand(string projectPath, string localUrl)
    {
        ...
        var exePath = Path.Combine(projectDirectory, "bin", "Debug", "net10.0", "Zayed.Api.exe");
        var dllPath = Path.Combine(projectDirectory, "bin", "Debug", "net10.0", "Zayed.Api.dll");
