...
namespace Zayed.Infrastructure.Vendors;
...
public sealed class InMemoryVendorRepository : IVendorRepository
{
    ...
    public Task<bool> ReversePaymentAsync(Guid id, decimal amount, CancellationToken cancellationToken = default)
    {
        ...
        vendor.ReversePayment(amount);
        return Task.FromResult(true);
    }

    public Task<bool> ApplyPurchaseReturnAsync(Guid id, decimal amount, CancellationToken cancellationToken = default)
    {
        if (!_vendors.TryGetValue(id, out var vendor))
        {
            return Task.FromResult(false);
        ...
        vendor.ApplyPurchaseReturn(amount);
        return Task.FromResult(true);
    }
