...
public sealed class SqliteDatabaseMaintenanceService : IDatabaseMaintenanceService
{
    ...
    private readonly ZayedDbContext _dbContext;
    ...
    private void ValidateBackupFile(string backupPath)
    {
        ...
        if (!hasAccountsTable)
        {
            throw new InvalidOperationException("Backup file is not a valid Zayed SQLite database.");
        }
    }
    ...
    private string BuildBackupPath(string? label, string backupKind)
    {
        ...
        var fileName = string.IsNullOrWhiteSpace(safeLabel)
            ? $"zayed-{backupKind.ToLowerInvariant()}-backup-{timestamp}.db"
            : $"zayed-{backupKind.ToLowerInvariant()}-backup-{timestamp}-{safeLabel}.db";
