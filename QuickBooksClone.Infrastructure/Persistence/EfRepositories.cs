...
public sealed class EfInvoiceRepository : IInvoiceRepository
{
    ...
    public async Task<InvoiceListResult> SearchAsync(InvoiceSearch search, CancellationToken cancellationToken = default) { var page = Math.Max(search.Page, 1); var pageSize = Math.Clamp(search.PageSize, 1, 100); var query = Query(); if (!search.IncludeVoid) query = query.Where(invoice => invoice.Status != InvoiceStatus.Void); if (search.CustomerId is not null) query = query.Where(invoice => invoice.CustomerId == search.CustomerId); if (!string.IsNullOrWhiteSpace(search.Search)) { var term = EfAccountRepository.Like(search.Search); query = query.Where(invoice => EF.Functions.Like(invoice.InvoiceNumber, term)); } var total = await query.CountAsync(cancellationToken); var items = await query.OrderByDescending(invoice => invoice.InvoiceDate).Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(cancellationToken); return new InvoiceListResult(items, total, page, pageSize); }
