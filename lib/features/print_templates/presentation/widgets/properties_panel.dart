...
class _PropertiesPanelState extends State<PropertiesPanel> {
  ...
  final _bindingController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: element == null ? _emptyState() : _elementEditor(element),
    );
  }
  ...
  Widget _emptyState() {
    return const Column(
      ...
      children: [
        ...
        SizedBox(height: 20),
        Text('Select an element on the page to edit its properties.', style: TextStyle(color: Color(0xFF64748B))),
      ],
    );
  }

  Widget _elementEditor(PrintElementModel element) {
    _valueController.text = element.value;
    _bindingController.text = element.binding ?? '';
      ...
      children: [
        ...
        if (element.type == 'text')
          TextField(
            ...
            onSubmitted: (value) => widget.controller.updateSelectedText(value: value),
          ),
        if (element.type == 'field') ...[
          TextField(
            controller: _bindingController,
            decoration: const InputDecoration(labelText: 'Binding', border: OutlineInputBorder()),
            onSubmitted: (value) => widget.controller.updateSelectedText(binding: value),
          ),
          const SizedBox(height: 8),
          DropdownButtonFormField<String>(
            ...
            items: TemplateFieldRegistry.fields
                .map((field) => DropdownMenuItem(value: field.key, child: Text(field.label)))
                .toList(),
            onChanged: (value) {
