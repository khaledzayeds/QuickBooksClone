        ...
        foreach (var line in request.Lines)
        {
            ...
            try
            {
                tax = ItemTypeBehavior.CanApplySalesTax(item.ItemType)
                    ? await ResolveTaxAsync(line.TaxCodeId, taxSettings, unitPrice, line.Quantity, discountPercent, cancellationToken)
            ...
            receipt.AddLine(new InvoiceLine(item.Id, description, line.Quantity, tax.NetUnitPrice, discountPercent, taxCodeId: tax.TaxCodeId, taxRatePercent: tax.RatePercent, taxAmount: tax.TaxAmount));
        }
