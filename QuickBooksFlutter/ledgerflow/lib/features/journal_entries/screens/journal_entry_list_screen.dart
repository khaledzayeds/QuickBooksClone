...
class _JournalEntryListScreenState
        ...
        child: Column(
          children: [
              ...
              child: Row(
                children: [
                  ...
                  _Tool(
                    ...
                    label: 'New',
                    onTap: () => context.push(AppRoutes.journalEntryNew),
                  ),
                  _Tool(
                    icon: Icons.refresh,
                    label: 'Refresh',
                          ...
                          Container(
                            ...
                            child: const Row(
                              children: [
                                ...
                                _HeaderCell('MEMO', flex: 5),
                                _HeaderCell('LINES', flex: 1, right: true),
            ...
            Container(
              ...
              child: Text(
                'Journal entries search  •  Enter opens workspace  •  Esc Close',
  ...
  List<JournalEntryModel> _filter(List<JournalEntryModel> entries) {
    final query = _queryCtrl.text.trim().toLowerCase();
    return entries.where((entry) {
      if (_selectedStatus != 0 && entry.status != _selectedStatus) {
        return false;
      }

      final range = _dateRange;
      if (range != null) {
        final date = DateUtils.dateOnly(entry.entryDate);
      ...
      return entry.entryNumber.toLowerCase().contains(query) ||
          entry.memo.toLowerCase().contains(query);
