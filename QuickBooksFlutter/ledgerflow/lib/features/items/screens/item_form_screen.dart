...
class _ItemFormScreenState extends ConsumerState<ItemFormScreen> {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<ItemType>(
                            ...
                            onChanged:
                                widget.isEdit &&
                                    (_loadedItem?.quantityOnHand ?? 0) != 0
                                ? null
                                : (value) {
                          ...
                          if (!widget.isEdit &&
                            ...
                            _InfoBox(
                              ...
                              text:
                                  'If opening quantity is greater than zero, the backend posts an opening inventory value. Purchase cost and Inventory Asset account are required.',
                            ),
                          ],
                          const SizedBox(height: 24),
                          _AccountsSection(
                            ...
                            onInventoryAssetChanged: (value) => setState(
                              () => _inventoryAssetAccountId = value,
                            ),
                            onCogsChanged: (value) =>
  ...
  void _applyDefaultAccountsIfEmpty({bool forceForType = false}) {
    ...
    setState(() {
      ...
      _incomeAccountId ??= find(
        ...
        ['sales income', 'service income', 'income'],
      );
  ...
  Future<void> _submit() async {
    ...
    if (accountError != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(accountError), backgroundColor: Colors.red),
      );
      return;
    }
...
class _AccountsSection extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        ...
        if (showSalesPurchase)
          _AccountPicker(
            label: itemType == ItemType.inventory
                ? 'Income Account *'
                : 'Income Account',
            value: incomeAccountId,
            accounts: _filter(accounts, [
        ...
        if (itemType == ItemType.bundle)
          _InfoBox(
            ...
            text:
                'Bundle items do not post directly. Component items should carry income, COGS, and inventory behavior.',
          ),
      ],
    );
  }

  static List<AccountModel> _filter(
  ...
  static String _ruleText(ItemType type) => switch (type) {
    ItemType.inventory =>
      'Inventory part requires Income, Inventory Asset, and COGS accounts.',
    ItemType.nonInventory =>
      'Non-inventory part can be sold, purchased, or both. Use Income and/or Expense account.',
    ItemType.service =>
      'Service can be sold, purchased, or both. Use Income and/or Expense account.',
    ItemType.bundle =>
      'Bundle/group is a container; posting should happen through component items.',
  };
}
...
class _ItemTypeHelp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final text = switch (type) {
      ItemType.inventory =>
        'Inventory Part tracks quantity on hand and posts to Inventory Asset and COGS.',
      ItemType.nonInventory =>
        'Non-inventory Part does not track stock; it can be bought, sold, or both.',
      ItemType.service =>
        'Service is labor or non-stock work; it can be sold or purchased.',
      ItemType.bundle =>
        'Bundle groups items on sales forms; accounting should flow through components.',
    };
    return Container(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Quantity on hand: ${item.quantityOnHand.toStringAsFixed(2)} ${item.unit ?? ''} • Status: ${item.isActive ? 'Active' : 'Inactive'}',
            ),
          ),
        ],
      ),
