...
class CustomerCreditListScreen extends ConsumerStatefulWidget {
  const CustomerCreditListScreen({super.key});

  @override
  ConsumerState<CustomerCreditListScreen> createState() =>
      _CustomerCreditListScreenState();
}

class _CustomerCreditListScreenState
    extends ConsumerState<CustomerCreditListScreen> {
        ...
        child: Column(
          children: [
              ...
              child: Row(
                children: [
                  ...
                  _Tool(
                    ...
                    label: 'New',
                    onTap: () => context.push(AppRoutes.customerCreditNew),
                  ),
                  _Tool(
                    icon: Icons.refresh,
                    label: 'Refresh',
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'Search ref #, customer, invoice...',
                          ...
                          Container(
                            ...
                            child: const Row(
                              children: [
                                ...
                                _HeaderCell('INVOICE / ACCT', flex: 3),
                                ...
                                return InkWell(
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      children: [
                                        _Cell(
                                          _fmtDate(credit.activityDate),
                                          flex: 2,
                                        ),
                                        _Cell(
                                          isRefund ? 'Refund' : 'Credit Memo',
                                          flex: 2,
                                        ),
                                        _Cell(
            ...
            Container(
              ...
              child: Text(
                'Customer credits search  •  Enter opens workspace  •  Esc Close',
  ...
  List<CustomerCreditModel> _filter(List<CustomerCreditModel> credits) {
    final query = _queryCtrl.text.trim().toLowerCase();
    return credits.where((credit) {
      if (_selectedStatus != 0 && credit.status != _selectedStatus) {
        return false;
      }
      ...
      return credit.referenceNumber.toLowerCase().contains(query) ||
          (credit.customerName ?? '').toLowerCase().contains(query) ||
          (credit.invoiceNumber ?? '').toLowerCase().contains(query) ||
          (credit.refundAccountName ?? '').toLowerCase().contains(query);
