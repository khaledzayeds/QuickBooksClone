...
class SalesOrdersRemoteDatasource {
  ...
  Future<ApiResult<SalesOrderModel>> getById(String id) async {
    try {
      final response = await _client.get<Map<String, dynamic>>('/api/sales-orders/$id');
  ...
  Future<ApiResult<SalesOrderModel>> close(String id) => _postAction(id, 'close');

  Future<ApiResult<SalesOrderModel>> cancel(String id) async {
    try {
      final response = await _client.patch<Map<String, dynamic>>('/api/sales-orders/$id/cancel');
      return Success(SalesOrderModel.fromJson(response.data!));
    } on DioException catch (error) {
      return Failure(parseError(error));
    }
  ...
  Future<ApiResult<SalesOrderModel>> _postAction(String id, String action) async {
    try {
      final response = await _client.post<Map<String, dynamic>>('/api/sales-orders/$id/$action');
      return Success(SalesOrderModel.fromJson(response.data!));
    } on DioException catch (error) {
      return Failure(parseError(error));
    }
