...
class ReceiveInventoryListScreen extends ConsumerStatefulWidget {
  const ReceiveInventoryListScreen({super.key});

  @override
  ConsumerState<ReceiveInventoryListScreen> createState() =>
      _ReceiveInventoryListScreenState();
}

class _ReceiveInventoryListScreenState
    extends ConsumerState<ReceiveInventoryListScreen> {
        ...
        child: Column(
          children: [
              ...
              child: Row(
                children: [
                  ...
                  _Tool(
                    ...
                    label: 'New',
                    onTap: () => context.push(AppRoutes.receiveInventoryNew),
                  ),
                  _Tool(
                    icon: Icons.refresh,
                    label: 'Refresh',
                    onTap: () => ref.invalidate(receiveInventoryListProvider),
                  ),
                  const Spacer(),
                  _Tool(
                    icon: Icons.close,
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'Search receipt #, vendor, notes...',
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        _Cell(receipt.vendorName, flex: 4),
                                        _CellWidget(
                                          flex: 2,
                                          child: Align(
                                            alignment: Alignment.centerLeft,
            ...
            Container(
              ...
              child: Text(
                'Inventory Receipts search  •  Enter opens receipt workspace  •  Esc Close',
  ...
  List<ReceiveInventoryModel> _filter(List<ReceiveInventoryModel> receipts) {
    final query = _queryCtrl.text.trim().toLowerCase();
    return receipts.where((receipt) {
      if (_selectedStatus != 'all' && receipt.status != _selectedStatus) {
        return false;
      }
      ...
      return receipt.receiptNumber.toLowerCase().contains(query) ||
          receipt.vendorName.toLowerCase().contains(query) ||
          (receipt.notes ?? '').toLowerCase().contains(query);
...
class _StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (Color bg, Color fg) = switch (status.toLowerCase()) {
      'posted' => (Colors.green.shade100, Colors.green.shade800),
      'void' => (Colors.red.shade100, Colors.red.shade800),
